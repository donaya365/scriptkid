...
surat=$(curl -sS https://raw.githubusercontent.com/donaya365/perizinan/main/authbot | grep -w $ipes | awk '{print $4}'  )
if [[ "$surat" = "true" ]]; then
echo -ne
else
...
wget -q --no-check-certificate "https://github.com/stedolan/jq/releases/download/jq-1.6/jq-linux64" -O /usr/local/sbin/jq
chmod +x usr/bin/jq
}
dircreate() {
...
dircreate
        ...
        echo -e "donaya365 Bot Panel Installer
        "
        [[ ! -f /root/ResBotAuth ]] && {
        ...
        echo ""
        echo -ne "Your name store [dafult: GEOVPN-STORE] : "
        read store_pnl
        [[ -z $store_pnl ]] && store_pnl="GEOVPN-STORE"
        echo ""
...
EOF
        ...
        fun_bot1() {
			...
			[[ "$(grep -wc "donaya365_bot" "/etc/rc.local")" = '0' ]] && {
			    sed -i '$ i\screen -dmS donaya365_bot bbt' /etc/rc.local >/dev/null 2>&1
			}
        }
        screen -dmS donaya365_bot bbt >/dev/null 2>&1
        fun_bot1
        [[ $(ps x | grep "donaya365_bot" | grep -v grep | wc -l) != '0' ]] && echo -e "\nBot successfully activated !" || echo -e "\nError1! Information not valid !"
        sleep 2
        menu
        } || {
       clear
        echo -e "Info...\n"
        fun_bot2() {
            screen -r -S "donaya365_bot" -X quit >/dev/null 2>&1
            [[ $(grep -wc "donaya365_bot" /etc/rc.local) != '0' ]] && {
                sed -i '/donaya365_bot/d' /etc/rc.local
            }
            rm -f /etc/.maAsiss/bot.conf
            sleep 1
        }
        fun_bot2
        echo -e "\nBot donaya365 berhenti!"
        sleep 2
        menu
    }
}
