...
IPs=$(cat /etc/xray/domain)
fi
[[ ! -d /var/lib/donaya365store ]] && exit 0
...
info_port() {
    [[ "${callback_query_from_id[$id]}" == "$Admin_ID" ]] || [[ "$(grep -wc ${callback_query_from_id} $User_Active)" != '0' ]] && {
        ...
        portsshws=$(grep -w "Websocket None TLS" /root/log-install.txt | awk '{print $5,$6}')
        ...
        OhpSSH=`cat /root/log-install.txt | grep -w "OHP_SSH" | cut -d: -f2 | awk '{print $1}'`
        OhpDB=`cat /root/log-install.txt | grep -w "OHP_Dropbear" | cut -d: -f2 | awk '{print $1}'`
        OhpOVPN=`cat /root/log-install.txt | grep -w "OHP_OpenVPN" | cut -d: -f2 | awk '{print $1}'`
        wsssl=`cat /root/log-install.txt | grep -w "Websocket TLS" | cut -d: -f2 | awk '{print $1}'`
...
EOF
    ...
    [[ "$t_time" == '1' ]] && hrs="hour" || hrs="hours"
        ...
        portsshws=`cat /root/log-install.txt | grep -w "Websocket None TLS" | cut -d: -f2 | awk '{print $1}'`
        wsssl=`cat /root/log-install.txt | grep -w "Websocket TLS" | cut -d: -f2 | awk '{print $1}'`
        OhpSSH=`cat /root/log-install.txt | grep -w "OHP_SSH" | cut -d: -f2 | awk '{print $1}'`
        OhpDB=`cat /root/log-install.txt | grep -w "OHP_Dropbear" | cut -d: -f2 | awk '{print $1}'`
        OhpOVPN=`cat /root/log-install.txt | grep -w "OHP_OpenVPN" | cut -d: -f2 | awk '{print $1}'`
        ...
        env_msg+="━━━━━━━━━━━━━━━━━━━━━\n"
        env_msg+="OpenVPN Config : http://$IPs:89/\n"
        env_msg+="━━━━━━━━━━━━━━━━━━━━━\n"
        env_msg+="Payload WS : \n\n"
...
$userna * $passw *
EOF
echo -e "### $userna $exp">>"/var/lib/donaya365store/data-user-sstp"
...
env_msg+="Cert : http://$IPs:89/server.crt\n"
env_msg+="━━━━━━━━━━━━━━━━━━━━━\n"
...
func_del_sstp() {
    userna=$1
    if [[ "${message_from_id[$id]}" = "$Admin_ID" ]]; then
        exp=$(grep -E "^###" /var/lib/donaya365store/data-user-sstp | cut -d ' ' -f 3 )
        sed -i "/^### $userna $exp/d" /var/lib/donaya365store/data-user-sstp
...
list_member_sstp() {
   if [[ "${callback_query_from_id[$id]}" == "$Admin_ID" ]]; then
      _result=$(grep -wE "^###" "/var/lib/donaya365store/data-user-sstp" | cut -d ' ' -f2 | column -t | sort | uniq | wc -l)
      _results=$(grep -wE "^###" "/var/lib/donaya365store/data-user-sstp" | cut -d ' ' -f2 | column -t | sort | uniq )
   elif [[ "${callback_query_from_id[$id]}" != "$Admin_ID" ]]; then
...
echo -e "### $userna $exp">>"/var/lib/donaya365store/data-user-l2tp"
...
func_del_l2tp() {
    export PATH="/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/local/sbin:/sbin:/bin"
    userna=$1
    if [[ "${message_from_id[$id]}" = "$Admin_ID" ]]; then
        exp=$(grep -E "^### " /var/lib/donaya365store/data-user-l2tp | cut -d ' ' -f 3 )
        sed -i '/^"'"$userna"'" l2tpd/d' /etc/ppp/chap-secrets
        sed -i '/^'"$userna"':\$1\$/d' /etc/ipsec.d/passwd
        sed -i "/^### $userna $exp/d" /var/lib/donaya365store/data-user-l2tp
        chmod 600 /etc/ppp/chap-secrets* /etc/ipsec.d/passwd*
        datata=$(find /etc/.maAsiss/ -name $userna)
        for accc in "${datata[@]}"
        do
...
list_member_l2tp() {
   if [[ "${callback_query_from_id[$id]}" == "$Admin_ID" ]]; then
      _result=$(grep -wE "^###" "/var/lib/donaya365store/data-user-l2tp" | cut -d ' ' -f2 | column -t | sort | uniq | wc -l)
      _results=$(grep -wE "^###" "/var/lib/donaya365store/data-user-l2tp" | cut -d ' ' -f2 | column -t | sort | uniq )
   elif [[ "${callback_query_from_id[$id]}" != "$Admin_ID" ]]; then
...
echo -e "### $userna $exp">>"/var/lib/donaya365store/data-user-pptp"
local env_msg
env_msg="━━━━━━━━━━━━━━━━━━━━━\n<b>         🔸 PPTP ACCOUNT 🔸 </b>\n━━━━━━━━━━━━━━━━━━━━━\n"
env_msg+="IP : $IPs\n"
env_msg+="Username : $userna\n"
...
func_del_pptp() {
    ...
    userna=$1
    if [[ "${message_from_id[$id]}" = "$Admin_ID" ]]; then
        exp=$(grep -E "^### " /var/lib/donaya365store/data-user-pptp | cut -d ' ' -f 3 )
        sed -i '/^"'"$userna"'" pptpd/d' /etc/ppp/chap-secrets
        sed -i "/^### $userna $exp/d" /var/lib/donaya365store/data-user-pptp
        chmod 600 /etc/ppp/chap-secrets* /etc/ipsec.d/passwd*
        datata=$(find /etc/.maAsiss/ -name $userna)
        for accc in "${datata[@]}"
        do
...
list_member_pptp() {
   if [[ "${callback_query_from_id[$id]}" == "$Admin_ID" ]]; then
      _result=$(grep -wE "^###" "/var/lib/donaya365store/data-user-pptp" | cut -d ' ' -f2 | column -t | sort | uniq | wc -l)
      _results=$(grep -wE "^###" "/var/lib/donaya365store/data-user-pptp" | cut -d ' ' -f2 | column -t | sort | uniq )
   elif [[ "${callback_query_from_id[$id]}" != "$Admin_ID" ]]; then
...
unset pub_menu
pub_menu=''
ShellBot.InlineKeyboardButton --button 'pub_menu' --line 1 --text '• SSH •' --callback_data 'ssh'
